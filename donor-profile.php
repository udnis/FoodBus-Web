...
if($type == "receiver"){
    header("Location:http://{$GLOBALS['ip']}/FoodWebApp/receiver-profile.php");
    exit();
}
...
if(isset($_POST['submit'])){
    ...
    if (!$conn) {
        ...
        $update = "UPDATE donor_credential SET email = '{$email}' WHERE user_id = {$id};";
        $update_address = "UPDATE donor_address SET name = '{$name}' , door = '{$door}' , street = '{$street}' ,area = '{$area}' ,village = '{$village}' ,city = '{$city}' ,pin = {$pin} ";
...
<head>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            document.getElementById("lat-pos").innerHTML = "Latitude: " + '<br/>' + <?= $donor_address[7]  ?>;
            document.getElementById("lng-pos").innerHTML = "Longitude: " + '<br/>' + <?= $donor_address[8] ?>;
...
<div class="con1 register-form-container">
    ...
    <form class="register-form" method="POST" submit="" id="register-form"  enctype="multipart/form-data">
        ...
        <input type="text" name="name" id="name" class="register-form-input register-name" value="<?= $donor_address[0] ?>" required>
        ...
        <input type="text" name="door" id="door" class="register-form-input register-door" value="<?= $donor_address[1] ?>"  required>

        <br><br>

        <label for="street" class="login-form-label">Edit Street</label>
        <input type="text" name="street" id="street" class="register-form-input register-street" value="<?= $donor_address[2] ?>"  required>

        <br><br>

        <label for="area" class="login-form-label">Edit Area</label>
        <input type="text" name="area" id="area" class="register-form-input register-area" value="<?= $donor_address[3] ?>"  required>

        <br><br>

        <label for="village" class="login-form-label">Edit Village/Town</label>
        <input type="text" name="village" id="village" class="register-form-input register-village" value="<?= $donor_address[4] ?>"  required>

        <br><br>

        <label for="city" class="login-form-label">Edit City</label>
        <input type="text" name="city" id="city" class="register-form-input register-city" value="<?= $donor_address[5] ?>"  required>

        <br><br>

        <label for="pin" class="login-form-label">Edit PIN</label>
        <input type="text" name="pin" id="pin" class="register-form-input register-pin" value="<?= $donor_address[6] ?>"  required>

        <br><br>

        <label for="phone" class="login-form-label">Edit Phone</label>
        <input type="text" name="phone" id="phone" class="register-form-input register-phone" value="<?= $donor_address[9] ?>"  required>
        ...
        <input type="hidden" id="latitude" name="latitude" value="<?= $donor_address[7] ?>" required/>
        <input type="hidden" id="longitude" name="longitude" value="<?= $donor_address[8] ?>" required/>
