...
if(isset($_GET['request_id']) && isset($_GET['rating'])){
    ...
    $get_receiver = "SELECT donor_id FROM food_details_receiver WHERE id = {$id};";
    ...
    while($row = mysqli_fetch_row($result4)){
        ...
        $prating = "SELECT rating,times FROM donor_address WHERE user_id={$receiver_id};";
        ...
        $rating_update = "UPDATE donor_address SET rating=($pre_rating + $rating),
                               times=($pre_times + 1) WHERE user_id = {$receiver_id}";

        $result5 = mysqli_query($conn,$rating_update);
        if($result5){
            $update = "UPDATE food_details_receiver SET receiver_complete = 1  WHERE id={$id}";
