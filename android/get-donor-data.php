...
if(isset($_GET['user_id'])){
    ...
    if(mysqli_num_rows($food_result) > 0){
        while($row = mysqli_fetch_row($food_result)){
            ...
            $food_wasted = "UPDATE food_details_receiver SET is_wasted = 1 WHERE food_id = {$f_id} AND is_wasted = 0";
    ...
    $data_name = "SELECT name,latitude,longitude,door,street,village,city,pin,phone,area,profile_url,rating,times from donor_address WHERE user_id = {$id} LIMIT 1;";
    $data_email = "SELECT email from donor_credential WHERE user_id = {$id} LIMIT 1;";
